...
def main():
    words = read_data()
    words = sorted(words,key=lambda temp:temp[1], reverse = True) # words.txt 단어를 빈도수 순으로 정렬합니다.
...
def read_data():
    ...
    line=open("words.txt")
    for i in line:
        words.append([i.split(",")[0],int(i.split(",")[1])])
    return words
...
def do_linear_regression(X, Y):
    ...
    lrmodel.fit(X,Y)
    slope = lrmodel.coef_[0] #0번째 변수 기울기
    intercept = lrmodel.intercept_#절편
