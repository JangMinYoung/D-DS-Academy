...
def read_data():
    ...
    '''
    sentences=open("ratings.txt")
    next(sentences)

    for line in sentences:
        if int(line.split("\t")[2])==1:
            training_sentences[1].append(line.split("\t")[1])
        else:
            training_sentences[0].append(line.split("\t")[1])
...
def calculate_doc_prob(training_sentence, testing_sentence, alpha):
    ...
    for token in training_model:
        sum_val+=training_model.get(token)

    for testing_token in testing_model:
        if testing_token in training_model:
            logprob+=math.log(training_model.get(testing_token)/sum_val)
        else:
            logprob+=math.log(alpha/sum_val)
    return logprob
